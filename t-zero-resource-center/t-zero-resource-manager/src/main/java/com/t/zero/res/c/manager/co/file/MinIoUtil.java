...
import com.t.zero.res.c.manager.service.EncodingUtils;
...
public class MinIoUtil {
     ...
     * @return Boolean
     */
    public void download(String userAgent, String range, String resInfoStore, int resInfoSize, HttpServletResponse res) {
        try {
    ...
    public void downloadLocal(File path, ResInfoDef resInfo) {
        try {
            var args = GetObjectArgs.builder().bucket(minioProperties.getBucketName()).object(resInfo.getResInfoStore()).build();
            IOUtils.copy(minioClient.getObject(args), new FileOutputStream(new File(path, EncodingUtils.toUtf(resInfo.getResInfoName()))));
        } catch (Exception e) {
            log.error("fileName: " + resInfo, e);
            throw new RuntimeException("file download failed");
        }
    }
